...
-- Since C_Timer.NewTicker doesn't exist in WotLK, use OnUpdate for polling
local function TargetIndicator_OnUpdate(self, elapsed)
    self._time = (self._time or 0) + elapsed
    if self._time > 0.1 then
        self._time = 0
        if self.__owner and self.__owner:IsShown() then
            TargetIndicator_Update(self.__owner)
        end
    end
end
...
local TargetIndicator_Start = function(self)
    ...
    if not overlay._onUpdateSet then
        overlay:SetScript("OnUpdate", TargetIndicator_OnUpdate)
        overlay.__owner = self
        overlay._onUpdateSet = true
    end
end

local TargetIndicator_Stop = function(self)
    local overlay = self.Overlay
    if overlay._onUpdateSet then
        overlay:SetScript("OnUpdate", nil)
        overlay._onUpdateSet = nil
        overlay._time = nil
        overlay:Hide()
    end
end
...
local UnitFrame_UpdateTextures = function(self)
    ...
    if (UnitIsPlayer(unit)) then
        ...
        if (not defaults.useStandardBossTexture) and (((UnitClassification(unit) == "worldboss") or (level < 1 and IsLevelAtEffectiveMaxLevel(level))) and (UnitCanAttack("player", unit))) then
            key = "Boss"
        elseif (not defaults.useStandardCritterTexture) and ((UnitCreatureType("target") == "Critter") or ((not ns.IsRetail) and (level == 1) and (UnitHealthMax(unit) < 30))) then
            key = "Critter"
        else
            key = (level < 1 or IsLevelAtEffectiveMaxLevel(level)) and "Seasoned" or level < 10 and "Novice" or
                "Hardened"
        end
    end
...
UnitStyles["Target"] = function(self, unit, id)
    ...
    -- Auras
    --------------------------------------------
    local auras = CreateFrame("Frame", "TargetAuras", self)
    auras:SetSize(unpack(db.AurasSize))
