...
local function createAuraIcon(element, index)
	--print all accessible properties of the element table
...
local function updateIcon(element, unit, index, offset, filter, isDebuff, visible)
	...
	if (name) then
		...
		local show = (element.CustomFilter or customFilter)(element, unit, button, name, rank, texture, count, debuffType,
			duration, expiration, caster, isStealable, shouldConsolidate, spellID)
...
local function UpdateAuras(self, event, unit)
	...
	if (auras) then
		...
		* unit - the unit for which the update has been triggered (string)
		...
		local visibleBuffs = filterIcons(auras, unit, auras.buffFilter or auras.filter or 'HELPFUL',
			math.min(numBuffs, max), nil, 0, true)
		...
		local visibleDebuffs = filterIcons(auras, unit, auras.debuffFilter or auras.filter or 'HARMFUL',
			math.min(numDebuffs, max - visibleBuffs), true, visibleBuffs)
		...
		* to   - the offset of the last aura button to be (re-)anchored (number)
		...
		if (fromRange or auras.createdIcons > auras.anchoredIcons) then
			...
			* to   - the offset of the last aura button to be (re-)anchored (number)
			--]]
			(auras.SetPosition or SetPosition)(auras, fromRange or auras.anchoredIcons + 1, toRange or auras
					.createdIcons)
	...
	if (buffs) then
		...
		if (fromRange or buffs.createdIcons > buffs.anchoredIcons) then
			(buffs.SetPosition or SetPosition)(buffs, fromRange or buffs.anchoredIcons + 1, toRange or buffs
					.createdIcons)
