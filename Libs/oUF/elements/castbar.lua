...
.isReverseFill	 - Indicates whether the castbar fills in reverse (boolean)
...
local function CastStart(self, event, unit)
	...
	local name, rank, text, texture, startTime, endTime, isTradeSkill, castID, notInterruptible, isChanneling =
		UnitCastingInfo(
			unit)
	event = 'UNIT_SPELLCAST_START'
	if (not name) then
		name, rank, isChanneling, texture, startTime, endTime, isTradeSkill, notInterruptible = UnitChannelInfo(unit)
	...
	element.isChanneling = isChanneling == "Channeling";
	...
	if (safeZone) then
		...
		if (element.channeling) then
			safeZone:SetPoint(element.isReverseFill and (isHoriz and 'RIGHT' or 'TOP') or
				(isHoriz and 'LEFT' or 'BOTTOM'))
		else
			safeZone:SetPoint(element.isReverseFill and (isHoriz and 'LEFT' or 'BOTTOM') or
...
local function CastUpdate(self, event, unit, spellName, rank, castID)
	if (self.unit ~= unit) then return end

	local element = self.Castbar
	if (not element:IsShown() or (not element.isChanneling and element.castID ~= castID)) then
		return
	end
...
local function CastStop(self, event, unit, spellName, rank, castID)
	if (self.unit ~= unit) then return end
...
local function CastFail(self, event, unit, spellName, rank, castID)
	if (self.unit ~= unit) then return end
		...
		if (self.Time) then
			if (self.delay ~= 0) then
				if (self.CustomDelayText) then
					...
					self.Time:SetFormattedText('%.1f|cffff0000%s%.2f|r', self.duration, isCasting and '+' or '-',
						self.delay)
				end
			else
