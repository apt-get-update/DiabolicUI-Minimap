...
local function Update(self, event, unit, absorb)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	local myIncomingHeal = ((HealComm:GetHealAmount(unitGUID, HealComm.ALL_HEALS, timeWindow, UnitGUID("player")) or 0) * (HealComm:GetHealModifier(unitGUID) or 1)) or
		0
	local allIncomingHeal = ((HealComm:GetHealAmount(unitGUID, HealComm.ALL_HEALS, timeWindow) or 0) * (HealComm:GetHealModifier(unitGUID) or 1)) or
		0
	...
	oUF.Tags.Methods["incomingheals:personal"] = function(tagUnit)
		local incHealP = (HealComm:GetHealAmount(UnitGUID(tagUnit), HealComm.ALL_HEALS, GetTime() + (element.lookAhead or 5), UnitGUID("player")) or 0) *
			(HealComm:GetHealModifier(UnitGUID(tagUnit)) or 1) or 0
		if incHealP > 0 then
			return E:ShortValue(incHealP)
		end
	end
	oUF.Tags.Methods["incomingheals:others"] = function(tagUnit)
		local incHealO = (HealComm:GetOthersHealAmount(UnitGUID(tagUnit), HealComm.ALL_HEALS, GetTime() + (element.lookAhead or 5)) or 0) *
			(HealComm:GetHealModifier(UnitGUID(tagUnit)) or 1) or 0
		if incHealO > 0 then
	...
	oUF.Tags.Methods["incomingheals"] = function(tagUnit)
		local incHeal = (HealComm:GetHealAmount(UnitGUID(tagUnit), HealComm.ALL_HEALS, GetTime() + (element.lookAhead or 5)) or 0) *
			(HealComm:GetHealModifier(UnitGUID(tagUnit)) or 1) or 0
		if incHeal > 0 then
	...
	* hasOverHealAbsorb - indicates if the amount of heal absorb is higher than the unit's current health (boolean)
	...
	if element.PostUpdate then
		return element:PostUpdate(unit, myIncomingHeal, otherIncomingHeal, absorb, healAbsorb, hasOverAbsorb,
			hasOverHealAbsorb, health, maxHealth)
	end
end
	...
	if element then
		...
		if element.overAbsorb then
			if element.overAbsorb:IsObjectType("Texture") and not element.overAbsorb:GetTexture() then
				element.overAbsorb:SetTexture(
					[[Interface\AddOns\ElvUI_ProjectZidras\Media\Textures\RaidFrame\Shield-Overshield]])
		...
		if element.overHealAbsorb then
			if element.overHealAbsorb:IsObjectType("Texture") and not element.overHealAbsorb:GetTexture() then
				element.overHealAbsorb:SetTexture(
					[[Interface\AddOns\ElvUI_ProjectZidras\Media\Textures\RaidFrame\Absorb-Overabsorb]])
